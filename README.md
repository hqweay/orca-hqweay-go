...
### 10. 🔗 链接元数据提取 (Link Metadata)

强大的网页元数据提取工具，支持将网页信息智能转换为 Orca 块属性：

- **智能提取**：
  - **自动模式**：在含有 URL 的块上使用提取命令，自动根据规则抓取标题、封面等信息。
  - **规则引擎**：支持自定义 URL 正则匹配规则，为不同网站编写专属 JavaScript 提取脚本。
  - **静态/动态**：默认使用静态抓取，支持回退到浏览器模式进行手动提取。
- **浏览器模式**：
  - **内置浏览器**：直接在插件内打开网页，所见即所得。
  - **交互式提取**：解决需要登录或动态渲染的页面提取难题。
  - **强力导航**：自动拦截 `target="_blank"` 跳转，确保始终在提取窗口内浏览。
- **配置与扩展**：
  - **快速访问**：内置常用网站（如豆瓣搜索）的快速入口。
  - **数据插入**：支持将元数据插入到当前块，或作为新块添加到“每日笔记”。
  - **顶部栏定制**：支持配置为顶部栏独立按钮或聚合菜单项。

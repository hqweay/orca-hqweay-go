...
17. 块工具箱？大幅增强引用块操作，一键将子块归集到源块并原地留痕；一键在父节点下对块置顶/置底；replace with 快捷转换。
...
21. Arc 侧边栏？类 Arc 浏览器的高效侧边栏，支持 Space 空间切换、已固定标签页（网格/列表双布局展示与拖拽排序）和 Today 历史记录（自动记录与去重），让多页面协作如丝般顺滑。
22. 侧边栏全局收集箱？完美复刻 Roam Research 侧边栏交互的全局单例收集箱，底层基于原生区块的完全持久化，随心所欲跨文档拖放收集，更有大纲折叠管理，跨页协作从未如此丝滑。
23. 链接工具？右键块链接或块引用，一键转换为块引用/块链接/文本📌/📌。
24. 大纲导航？全局侧边栏大纲树，支持双剑合一的全文检索、精准高亮、无感截断显示与左右键分离的下钻交互，重塑长篇文档的阅读体验。
...
- **FSRS 算法集成**：集成 [ts-fsrs](https://github.com/open-spaced-repetition/ts-fsrs)，根据遗忘曲线智能调度复习任务。
- **漫游模式 (Roaming Mode)**：
...
- **右键即刻漫步**: 右键点击任意父块或查询块即可直接开启临时漫步，无需打标。可在设置中配置显示该右键选项所需的最小子块数量。
...
- **归集子块菜单**：将以下三个归集功能收纳于「归集子块到引用源」二级菜单下，使右键菜单更简洁：
  - **仅归集**：将当前引用块下的子块移动到其指向的源块中，保留当前引用关系。
  - **归集并删除**：将子块归集到源块后，直接删除当前的引用块（用于临时中转页面的清理）。
  - **归集并留痕**：将子块移动到源块，同时将当前引用块转换为纯文本（保留别名），并在其末尾追加一个指向源块的 `📌` 块链接图标，在其下方生成指向已移动子块的新引用，实现“数据入库，原地留踪，且支持回导”。
- **在父块下置顶/置底**：快速将选中的块移动 to 其父块的开头或末尾，方便进行版面整理。
- **引用与链接转换**：
  - **块引用转块链接**：一键将块引用批量转换为常规超链接（`[文本](orca://block/block-id)`），若别名为空则智能获取原块文本。
  - **块引用转文本📌**：在别名或文本末尾添加 `📌` 后缀，形成更轻量直观的文本图钉引用。
  - **块引用转📌**：将别名设为单个 `📌`，形成纯图标图钉引用。
  - **块链接转块引用**：将符合格式的超链接批量无损转换回原生块引用，保留当前文本作为别名。
...
- **快捷交互**：
  - **左键点击**：一键在右侧边栏重新打开**上一次打开的置顶块**，适合快速切回核心工作区。
  - **右键点击**：弹出所有置顶块的完整列表，供自由选择。
- **自定义命名**：支持通过在标签上添加 `displayName` 属性来自定义该块在面板中的显示名称，保持列表整洁。

### 21. 🪐 Arc 侧边栏 (Arc Tabs)

为虎鲸笔记提供媲美 Arc 浏览器的现代化标签页与侧边栏管理系统，告别杂乱标签页，打造极致顺滑的多任务工作流：

- **沉浸式多空间 (Spaces)**：支持创建多个独立的空间（默认 🪐），底部图标完美兼容纯 Emoji 或文字首字母。各空间之间“已固定”块完全隔离。
- **📌 已固定标签页 (Pinned Tabs)**：
  - **标签化存储**：使用 `#ArcTab` 标签（可在设置中自定义）持久化存储固定状态，天然融入知识双链。
  - **拖拽固定**：支持直接选中编辑器中任意 Block，拖拽并落入侧边栏固定区实现秒级固定。
  - **网格/列表双布局**：支持紧凑网格（类似移动端 App 图标库）与详细列表（经典垂直列表）两种模式。
  - **拖拽排序**：支持在侧边栏自由拖拽已固定的标签页进行排序，顺序自动保存至插件设置中。
- **🕒 今天访问记录 (Today Tabs)**：
  - **实时同步无缝感知**：自动收集当前激活及最近浏览过的 Block，并与已固定内容去重。
  - **安全防抖的关闭逻辑**：点击侧边栏的“关闭”仅从视觉和历史中抹去记录，不强杀底层核心面板，保护工作现场。
  - **离线秒开缓存**：利用 LocalStorage 缓存图标和标题，重启或断网也能瞬间载入。
- **✨ 媲美原生的极致微动画**：
  - 为网格模式独家定制了 `ZoomIn` 轻盈弹出动画，为列表模式配置了 `SlideIn` 平滑推挤展开动画。
  - 深度解耦底层状态刷新逻辑，配合 CSS 纯手写交互补间动画（`0.3s ease`），彻底告别生硬闪烁与跳跃。
- **灵动的左右停靠**：
  - **设置项默认方向**：支持在插件设置中配置侧边栏默认停靠在**左侧**或**右侧**。
  - **快捷反向打开**：无论你的默认设置在哪边，只要**右键点击**顶栏按钮，或是**按住 Shift 点击**，侧边栏就会如同变魔术般在屏幕的反方向弹出。

### 22. 📥 侧边栏收集箱 (Roam Sidebar)

为虎鲸笔记引入 Roam Research 经典的右侧边栏"全局收集箱"交互范式，让跨文档的素材摘录与组织前所未有的顺滑：

- **全局唯一的"知识手提箱"**：点击顶部栏图标即可呼出，无论你在哪篇文档，侧边栏永远是同一个专用的底层块（Roam Sidebar），告别冗余的碎片垃圾块。
- **万物皆可拖拽的"沉浸流"**：支持将编辑器中任意的文本、区块，直接跨屏拖放到底部的空白区域即可追加。底层采用防抖与防闪烁逻辑，全屏幕都是着陆区。
- **原汁原味的 Block 级渲染**：不同于死板的卡片列表，侧边栏内是真正的原生编辑器！不仅完美支持块菜单、拖拽把手与斜杠命令，甚至支持将一个块直接拖拽给侧边栏里的另一个块，原生完成缩进和层级嵌套。
- **原生图谱级数据持久化**：不依赖易失的内存或本地存储。侧边栏的状态（排列、折叠）在发生变化时会实时写入该专用底层块的原生 `_repr` 属性中，即便重启软件或跨设备，你的专属收集状态也永不丢失。
- **一键大纲化 (Collapse All)**：支持在侧边栏顶部一键"展开/折叠"所有内容。折叠时巧妙地保留了根块的文本本身，仅隐藏其庞大的子级树，秒变清爽的大纲目录。

### 23. 🔗 链接工具 (Link Tools)

右键块链接或块引用时，在虎鲸笔记原生菜单中注入快捷转换选项：

- **右键块链接**：一键将超链接转换为原生块引用。
- **右键块引用**：
  - 转换为块链接（`[文本](orca://block/block-id)`）。
  - 转换为文本📌（轻量图钉引用）。
  - 转换为📌（纯图标图钉引用）。
- **智能识别**：仅对块链接和块引用生效，普通链接不受影响。

### 24. 🌳 大纲导航 (Block Nav)

为虎鲸笔记引入现代化、结构清晰的全局侧边栏大纲树，专为阅读与整理长篇文档而生：

- **全局视图与下钻 (Global View & Drill-down)**：
  - **左键平滑跳转**：点击大纲节点，右侧编辑器瞬间聚焦，侧边栏本身保持稳定，绝不乱跳。
  - **右键专注下钻**：右键点击任意节点，立即将整个侧边栏的根节点切换为该块，完美摒弃多余干扰，专注当前子树。
- **无死角的全文检索 (Full-text Search)**：
  - 侧边栏自带专属搜索框，支持大小写不敏感的实时本地过滤。
  - **双剑合一匹配**：彻底打通了底层检索逻辑与视觉渲染逻辑，即使块使用了自定义 `displayName` 属性或是日期格式，也能做到“所搜即所见，所见即能搜”。
  - **精准高亮**：匹配命中的关键词会在大纲中被精准涂上醒目的黄色高亮笔刷。
- **完美的文字展示体验**：
  - **动态截断**：告别生硬的 JavaScript 定长截断，利用 CSS 弹性布局，侧边栏多窄都不换行。
  - **原生悬浮提示**：即使文字再长，鼠标悬浮也能通过浏览器原生 tooltip 看到一字不落的完整节点内容。
- **原生 UI 无缝嵌入**：
  - 紧贴着原生的左侧边栏切换按钮，天然融入 Orca 的顶部栏生态。
- **灵动的左右停靠**：
  - 支持在设置中配置大纲树默认在**左侧**或**右侧**打开。
  - 同样支持 **右键点击** 或 **Shift + 点击** 顶栏图标，瞬间在屏幕对立面唤出大纲！

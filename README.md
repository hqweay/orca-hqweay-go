...
### 21. 🗂️ Arc 侧边栏 (Arc Tabs)

为虎鲸笔记提供媲美 Arc 浏览器的现代化标签页与侧边栏管理系统，助力多任务工作流：

- **Space 空间管理**：支持创建多个空间（Spaces），如“个人”、“工作”等，不同空间拥有完全独立的 Pinned（已固定）标签页集合。
- **已固定标签页 (Pinned Tabs)**：
  - **标签化存储**：使用 `#ArcTab` 标签（可在设置中自定义）持久化存储固定状态，天然融入你的知识网络中。
  - **拖拽固定**：支持直接从编辑器中拖拽任意 Block 节点并松手落入侧边栏来快速固定。
  - **网格/列表双布局**：支持紧凑网格（仅显示图标，无图标时提取首字大写居中）与详细列表（显示标题与图标）两种渲染模式。
  - **拖拽排序**：支持在侧边栏拖拽已固定笔记进行自定义排序，顺序实时保存至插件设置中。
  - **静默后台打标**：对未打开的笔记进行固定/取消固定时，自动在后台以透明面板方式静默打开打标并关闭，用户无感知。
- **今天访问记录 (Today Tabs)**：
  - **动态组合**：实时获取当前屏幕上所有打开的面板块，并与最近访问历史记录（基于 LocalStorage 缓存，最多 15 条，去重）合并展示。
  - **自动缓存**：缓存 Block 的图标和标题，保证即使在离线或插件重启时也能一瞬间以最完美的视觉样式载入。
- **极致响应体验**：核心状态基于 Valtio 零延迟响应式状态代理，操作反馈达到 0ms 级别。

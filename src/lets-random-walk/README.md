...
- **快捷键支持**: 支持在设置中为“随机漫步”配置快捷键，一键继续未完的探索。
...
4. 点击顶部 Headbar 的 🎲 骰子图标即可漫游你上次看过的频道，或在图标上右键点击以打开菜单切换频道。
5. **临时漫步**: 在任意感兴趣的块上右键选择“随机漫步”，即可直接开始探索该块的内部世界。此时顶部的骰子图标也会自动记忆并锁定到该块。

## ⚙️ 配置项

- **漫步标签**: 用于识别随机漫步频道的标签名称，默认为 `#随机漫步`。
- **右键菜单最小子块数量**: 用于限制右键菜单中显示“随机漫步”选项所需要的最小子块数量，默认为 `2`。这样能有效避免在只有一个子块或没有子块的父块上显示多余的菜单项。

...
const logger = new Logger("srs:tag-schema");
...
export async function getOrCreateTagBlock(
  ...
  try {
    ...
    logger.debug(`[${pluginName}] Tag ${alias} not found, creating...`);
    ...
    logger.error(`[${pluginName}] Failed to create ${alias} tag block.`);
    return null;
  } catch (error) {
    logger.error(`[${pluginName}] Failed to get/create tag ${alias}:`, error);
    return null;
  }
}

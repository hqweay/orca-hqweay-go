...
const logger = new Logger("srs:storage");
...
export async function ensureCardTag(card: SrsCardData): Promise<void> {
  logger.debug("ensureCardTag", card);
...
export async function revertCardToState(card: SrsCardData): Promise<void> {
  if (
    ...
    logger.warn("No snapshot found for card undo", card.blockId);
    return;
  }
...
export async function updateCardProperties(
  ...
  if (card.cardRef) {
    try {
      logger.debug("updateCardProperties", card.cardRef, tagProperties);
      ...
      logger.warn("setRefData failed, falling back to insertTag", e);
    }
  }
  ...
  logger.debug("updateCardProperties", card);
}

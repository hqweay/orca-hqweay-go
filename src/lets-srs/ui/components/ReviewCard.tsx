...
const logger = new Logger("srs:review-card");
...
export const ReviewCard: React.FC<ReviewCardProps> = ({
  ...
  const handleGradeAction = async (grade: CardGrade) => {
    ...
    try {
      ...
      logger.error("failed to save card review", err);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handlePostpone = async () => {
    ...
    try {
      ...
      logger.error("failed to postpone card", err);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleToggleStatus = async (status: string) => {
    ...
    try {
      ...
      logger.debug("handleToggleStatus", activeCard);
      ...
      logger.error(`failed to toggle status: ${status}`, err);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSaveRemark = async () => {
    ...
    try {
      ...
      logger.error("failed to save remark", err);
      setIsSaving(false);
    }
  };
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      logger.error("failed to upgrade card", err);

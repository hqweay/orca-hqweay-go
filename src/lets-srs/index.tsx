...
export default class SrsPlugin extends BasePlugin {
  ...
  async load() {
    ...
    this.registerBlockConverter("plain", RENDERER_TYPE, (block, repr) => {
      // 返回代表这个块的纯文本字符串。
                  ...
                  if (blockIds.length === 1) {
                    ...
                    if (repr?.type === "query") {
                      try {
                        ...
                        this.logger.debug("queryDescription", queryDescription);
                        ...
                        this.logger.error("Failed to query items", err);
                      }
                    }
                    ...
                    try {
                      ...
                      this.logger.error("smart roam failed", err);
                      this.handleRoam([blockId]);
                    }
                  } else {
                    // 否则漫游所有选中的块
